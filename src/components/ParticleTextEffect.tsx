...
export function ParticleTextEffect({ words = DEFAULT_WORDS, cycleInterval = 240 }: ParticleTextEffectProps) {
  ...
  useEffect(() => {
    ...
    const nextWord = (word: string) => {
      ...
      // Dynamic font size for perfect fit on any screen
      const fontSize = Math.max(70, Math.min(130, Math.min(canvas.width, canvas.height) / 7));
      offscreenCtx.font = `bold ${fontSize}px system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif`;
      ...
      // Random vibrant color for every word
      const newColor = {
    ...
    const resize = () => {
      canvas.width = window.innerWidth;
      canvas.height = window.innerHeight;
      nextWord(wordsRef.current[wordIndexRef.current] || wordsRef.current[0] || "HI");
    };
...
ctx.fillRect(0, 0, canvas.width, canvas.height);
    ...
    nextWord(wordsRef.current[0] || "HI");
    animate();
